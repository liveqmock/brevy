 ...
 * @author caobin
 */
Ext.define("App.systemMGR.authCFG.userGroup.refRole.RefRole", {
	extend : "App.Module",
	...
	//创建用户组信息显示Panel
	createUserGroupInfoPanel : function(){
		return Ext.create("Ext.panel.Panel", {
	...
	createSelectedGrid : function(){
		return Ext.create("Ext.grid.Panel", {
			...
			store: userGroupRefRoleSelectedDS,
			viewConfig: {
		        stripeRows: true
		    },
	...
	createCandidateGrid : function(){
		return Ext.create("Ext.grid.Panel", {
			...
			store: userGroupRefRoleCandidateDS,
			viewConfig: {
		        stripeRows: true,
		        forceFit: true
		    },
		...
		return [
			{text: this.addRefs, iconCls: Ext.ux.Icons.fugue_plus_octagon, handler: function(btn){
				Pub.MsgBox.showMsgBox(Pub.MsgBox.CONFIRM, me.confirmAddAllRefs, null, function(c){
					if(c == "yes"){
						var selection = Ext.getCmp("UserGroupReadMainGridID").getSelectionModel().getSelection()[0];
						var count = userGroupRefRoleCandidateDS.getCount();
						var roleIds = [];
						if(count > 0){
							userGroupRefRoleCandidateDS.each(function(r){
								roleIds.push(r.get("id"))
							});
							Ext.Ajax.request({

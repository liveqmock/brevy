/*
 * Tree combo Use with 'Ext.data.TreeStore'
 * 
 * 
 * If store root note has 'checked' property tree combo becomes multiselect
 * combo (tree store must have records with 'checked' property)
 * 
 * 
 * Has event 'itemclick' that can be used to capture click
 * 
 * 
 * Options: selectChildren - if set true and if store isn't multiselect,
 * clicking on an non-leaf node selects all it's children canSelectFolders - if
 * set true and store isn't multiselect clicking on a folder selects that folder
 * also as a value
 * 
 * 
 * Use:
 * 
 * 
 * single leaf node selector: selectChildren: false canSelectFolders: false -
 * this will select only leaf nodes and will not allow selecting non-leaf nodes
 * 
 * 
 * single node selector (can select leaf and non-leaf nodes) selectChildren:
 * false canSelectFolders: true - this will select single value either leaf or
 * non-leaf
 * 
 * 
 * children selector: selectChildren: true canSelectFolders: true - clicking on
 * a node will select it's children and node, clicking on a leaf node will
 * select only that node
 * 
 * 
 * This config: selectChildren: true canSelectFolders: false - is invalid, you
 * cannot select children without node
 * 
...
Ext.define('Ext.ux.TreeCombo', {
	...
	setValue : function(valueInit) {
		...
		valueInit += "";
		var me = this, tree = this.tree, values = (valueInit == '')
				? []
				: valueInit.split(','), valueFin = [];
		...
		Ext.each(me.recursiveRecords, function(record) {
			...
			//add by caobin
			if (!me.multiselect && id == valueInit){			
	...
	checkParentNodes : function(node) {
		...
		node.eachChild(function(nodesingle) {
					var id = nodesingle.getId(), index = me.ids
							.indexOf('' + id);
	...
	initComponent : function() {
		...
		me.tree = Ext.create('Ext.tree.Panel', {
					...
					hidden : true,
					minHeight : 200,
					rootVisible : (typeof me.rootVisible != 'undefined')
							? me.rootVisible
							: true,
					floating : true,
					useArrows : true,
					...
					listeners : {
						...
						itemclick : function(view, record, item, index, e,
								eOpts) {
							me.itemTreeClick(view, record, item, index, e,
									eOpts, me)
						}
					}
				});
	...
	itemTreeClick : function(view, record, item, index, e, eOpts, treeCombo) {
		var me = treeCombo, checked = !record.get('checked');// it is still
		// not checked
		// if will be
		// checked in
		// this event
		...
		var node = me.tree.getRootNode().findChild(me.valueField,
				record.get(me.valueField), true);
		if (node == null) {
			if (me.tree.getRootNode().get(me.valueField) == record
					.get(me.valueField))
		...
		// if it can't select folders and it is a folder check existing values
		// and return false

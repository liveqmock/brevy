(function() {
	 ...
	 *	Usage:
     *
	 * 	Add the plugin file to your JavaScript sources: ClearButton.js
	 * 	Add the corresponding CSS file: ClearButton.css
	 * 	Add the plugin alias "clearbutton" to the "plugins" config option of the field:
     *
	 *	{
	 *		xtype: 'textfield',
	 *		fieldLabel: 'Search',
    ...
    Ext.define('Ext.ux.form.field.ClearButton', {
        ...
        calculateClearButtonPosition: function(textField) {
        	//fixed position under ie browser //by caobin 
        	var pos;
            switch(this.cmp.xtype){
            	case "iconcombo":
            		pos = 16;//((Ext.isIE || Ext.isChrome) ? 16 : 0);//FF高版本已解决位置问题
            		break;
            	default:
            		pos = -2;//((Ext.isIE || Ext.isChrome) ? -2 : -18);
            	
            }
            var positions = textField.inputEl.getBox(true, true);
            var top = positions.y - (Ext.isIE ? 1 : 0);
            var right = positions.x - pos;

...
public interface CadGdDao extends CrudRepository<CadGd, Long> {
	 ...
	 * @author caobin
	 */
	@Query("select a from CadGd a, CadRefDeptGd b where a.id=b.id.gdId and b.id.deptId=:deptId and (lower(a.name) like lower(:kw) or lower(a.briefName) like lower(:kw) or lower(a.implTeam) like lower(:kw) or lower(a.pmName) like lower(:kw)) and a.monitor=:monitor order by a.id")
	Page<CadGd> searchByKeyword(@Param("deptId")long deptId, @Param("kw")String keyword, @Param("monitor")String monitor, Pageable pageable);
